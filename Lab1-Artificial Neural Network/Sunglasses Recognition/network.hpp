...
private:
    ...
    double learning_rate = 0.15, momentum = 0.8, ***w, ***dw, **b, **net, ***global_dw, **global_db, **sigma, ***best_w, **best_b, ***v, **vb;
...
public:
    ...
    void predict(double *x, double **result) {
        ...
        //for output unit, use logistic
        result[0][0] = 1.0 / (1.0 + exp(-net[layers - 2][0]));
    ...
    void back_propagation(double *x, double *correct) {
        ...
        for (int now = 0; now < net_struct[layers - 1]; now++) {
            ...
            //computing sigma
            //this time, use cross-entropy loss function
    ...
    bool evaluate_performance(double &error, double* input, double* target) {
        ...
        error = -(target[0] * log(result[0][0]) + (1.0 - target[0]) * log(1.0 - result[0][0]));
    ...
    void real_predict(char* filename) {
        ...
        cout << "The neural network predicts that ";
        if (result[0][0] > 0.5)
            cout << "the person wears a sunglasses." << endl;
        else
            cout << "the person does not wear a sunglasses." << endl;

        cout << endl;

        if (((target[0][0] > 0.5) && (result[0][0] > 0.5)) || ((target[0][0] < 0.5) && (result[0][0] < 0.5))){
